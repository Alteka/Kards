...
export default {
  ...
  computed: {
    infoStyle: function () {
      ...
      if (
        visualViewport.height >= this.config.led.rows * this.config.led.height &&
        this.config.led.height > 42
      ) {
      ...
      return {
        left:
          Math.min(visualViewport.width, this.config.led.columns * this.config.led.width) / 2 +
          'px',
        top: top + 'px',
        fontSize: offset + 'px'
      }

<template>
  <div id="placeholder" :style="{ background: config.placeholder.bg }">
    <transition name="fade">
      <div v-if="config.placeholder.gradient" id="gradient"></div>
    </transition>
    <div
      v-resize-text="{ ratio: 1.1, minFontSize: '10px', maxFontSize: '500px' }"
      class="name"
      :style="{ color: config.placeholder.fg }"
    >
      <i
        v-if="config.placeholder.icon != 'custom'"
        style="font-size: 200%"
        class="fa-solid"
        :class="config.placeholder.icon"
      ></i>
    ...
    <transition name="fade">
      <div
        v-if="config.showInfo"
        id="infoText"
        v-resize-text="{ ratio: 4 }"
        :style="{ color: config.placeholder.fg }"
      >
        {{ info.cardSize }}
      </div>
